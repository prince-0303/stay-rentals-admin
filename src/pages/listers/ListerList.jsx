...
import { getListers } from "@/api/users";
...
const columns = (navigate) => [
    ...
    },
    {
        ...
        cell: ({ row }) => (
            <button
                onClick={() => navigate(`/listers/${row.original.id || row.original._id}`)}
...
export default function ListerList() {
    const navigate = useNavigate();
    const { data, isLoading } = useQuery({
        queryKey: ["listers"],
        queryFn: () => getListers().then((r) => r.data?.results || r.data?.listers || r.data || []),
        retry: false,
    });
    ...
    return (
        <div className="space-y-6">
            <div>
                <h2 className="text-xl font-bold text-foreground">Lister Management</h2>
                <p className="text-sm text-muted-foreground mt-0.5">Manage all registered property listers</p>
            </div>
            <DataTable
                data={data || []}
